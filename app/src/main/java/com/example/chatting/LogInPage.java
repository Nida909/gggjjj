...
public class LogInPage extends AppCompatActivity {
    ...
    public void login(View view)
    {
        ...
        else
        {
            ...
            {
                String[] columns = {DatabaseContract.Customers._ID, "Name", "Password"};
                Cursor cr=db.query(DatabaseContract.Customers.TABLE_NAME,columns,"Name=? AND Password=?",values,null,null,null);
                if (cr.getCount() > 0) {
                    cr.moveToFirst();
                   // bo = true;
                ...
                String[] columns = {DatabaseContract.Riders._ID, "Name", "Password"};
                Cursor cr=db.query(DatabaseContract.Riders.TABLE_NAME,columns,"Name=? AND Password=?",values,null,null,null);
                if (cr.getCount() > 0) {
                    cr.moveToFirst();
                    // bo = true;
                    val = String.valueOf(cr.getLong(0));
